...
export default function Profit() {
  ...
  useEffect(() => {
    setLoading(true);
    setData(null);
    axios.get(`/api/profit?period=${period}`)
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase',
        ...
        SALES PROFIT — {periodLabel.toUpperCase()}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 24 }}>
        ...
        <div className="stat-card">
          <div className="stat-info">
            <p>Revenue</p>
            <h3>{fmt(data?.revenue)}</h3>
            <small style={{ color: 'var(--text-muted)', fontSize: 11 }}>
              {data?.sales_count || 0} sale{data?.sales_count !== 1 ? 's' : ''}
            </small>
          </div>
        ...
        <div className="stat-card">
          <div className="stat-info">
            <p>Profit Earned</p>
            <h3 style={{ color: 'var(--green)' }}>{fmt(data?.profit)}</h3>
            <small style={{ color: 'var(--text-muted)', fontSize: 11 }}>After cost deduction</small>
          </div>
        ...
        <div className="stat-card">
          <div className="stat-info">
            <p>Profit Margin</p>
            <h3 style={{ color: marginColor(data?.profit_margin || 0) }}>
              {data?.profit_margin || 0}%
            </h3>
            <small style={{ color: 'var(--text-muted)', fontSize: 11 }}>On sales made</small>
          </div>
      ...
      {/* No sales message */}
      {data?.sales_count === 0 && (
        <div style={{ background: '#fff3e0', border: '1px solid #ffe0b2', borderRadius: 'var(--radius)',
          padding: '14px 18px', marginBottom: 20, color: '#e65100', fontSize: 13 }}>
          ⚠ No sales recorded for <strong>{periodLabel}</strong>. Revenue and profit will show as ₹0.00.
        </div>
      )}
      ...
      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase',
        color: 'var(--text-muted)', marginBottom: 12 }}>
        INVENTORY PROFIT POTENTIAL — ALL STOCK
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 24 }}>
        <div className="stat-card">
          <div className="stat-info">
            <p>Total Cost</p>
            <h3 style={{ fontSize: 18 }}>{fmt(data?.total_cost)}</h3>
            <small style={{ color: 'var(--text-muted)', fontSize: 11 }}>What you paid</small>
          </div>
          <div className="stat-icon" style={{ background: '#ffeaea' }}>
        ...
        <div className="stat-card">
          <div className="stat-info">
            <p>Selling Value</p>
            <h3 style={{ fontSize: 18 }}>{fmt(data?.selling_value)}</h3>
            <small style={{ color: 'var(--text-muted)', fontSize: 11 }}>If all sold</small>
          </div>
          <div className="stat-icon" style={{ background: '#e3f0ff' }}>
        ...
        <div className="stat-card">
          <div className="stat-info">
            ...
            <h3 style={{ fontSize: 18, color: 'var(--green)' }}>{fmt(data?.potential_profit)}</h3>
            <small style={{ color: 'var(--text-muted)', fontSize: 11 }}>If all sold</small>
          </div>
          <div className="stat-icon" style={{ background: 'var(--green-light)' }}>
        ...
        <div className="stat-card">
          <div className="stat-info">
            <p>Avg Margin</p>
            <h3 style={{ fontSize: 18 }}>{data?.avg_margin || 0}%</h3>
            <small style={{ color: 'var(--text-muted)', fontSize: 11 }}>Across all products</small>
          </div>
          <div className="stat-icon" style={{ background: '#f3e8ff' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
          ...
          {(data?.breakdown || []).slice(0, 5).map((p, i) => (
            <div className="list-item" key={i}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ width: 24, height: 24, borderRadius: '50%', background: 'var(--green-light)',
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>
                    Cost ₹{p.cost_price} → Sell ₹{p.price}
                  </div>
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <span style={{ background: marginBg(p.margin), color: marginColor(p.margin),
                  padding: '2px 8px', borderRadius: 20, fontSize: 11, fontWeight: 700 }}>
                  {p.margin}%
                </span>
                <div style={{ fontSize: 11, color: 'var(--green)', marginTop: 2 }}>
                  +₹{p.profit_per_unit}/unit
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          {(data?.breakdown || []).slice(-5).reverse().map((p, i) => (
            <div className="list-item" key={i}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ width: 24, height: 24, borderRadius: '50%', background: 'var(--red-light)',
      ...
      <div className="card" style={{ padding: 20 }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#f57c00" strokeWidth="2">
